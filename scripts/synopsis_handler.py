# scripts/synopsis_handler.py

import json
from pathlib import Path
from bs4 import BeautifulSoup

def process_synopsis_images(crawler, synopsis, data_dir, callbacks=None):
    from .crawler_base import Chapter
    temp_chapter = Chapter(id=0)
    temp_chapter.body = synopsis
    temp_chapter.images = {}
    crawler.output_path = str(data_dir)
    crawler.extract_chapter_images(temp_chapter, callbacks=callbacks)
    if callbacks:
        callbacks.on_log(f"DEBUG: process_synopsis_images извлекла {len(temp_chapter.images)} изображений")
